...
export default function Game() {
  ...
  useEffect(() => {
    ...
    newSocket.on('gameStarted', ({ roomId, question, currentQuestionId, totalQuestions }) => {
      setRoomId(roomId);
    ...
    newSocket.on('answerResult', ({ isCorrect, explanation, message }) => {
      if (isCorrect) {
    ...
    newSocket.on('questionUpdate', ({ question, currentQuestionId, totalQuestions }) => {
      setCurrentQuestion(question);
      setCurrentQuestionId(currentQuestionId);
      setTotalQuestions(totalQuestions);
      setShowExplanation(false);
    ...
    newSocket.on('gameComplete', ({ message, totalQuestions }) => {
      setIsComplete(true);
      setMessage(message);
    });
  ...
  const handleAnswer = (answer) => {
    if (socket && roomId && !showExplanation) {
